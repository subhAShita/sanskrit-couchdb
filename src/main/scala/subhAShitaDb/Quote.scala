...
case class QuoteText(scriptRenderings: List[ScriptRendering],
                     var key: String = "",
                     language: Language = subhAShitaDb.Language("UNK"),
                     metre: String = "UNK") {
  val log = LoggerFactory.getLogger(getClass.getName)
  ...
  def this(text: String, script: String, language: Language) =
    this(scriptRenderings=List(text).filter(_.nonEmpty).map(x => subhAShitaDb.ScriptRendering(text=x,scheme=script)),
      language=language)
  def this(text: String) = this(text=text, script=transliterator.scriptUnknown, language = subhAShitaDb.Language("UNK"))
}
...
case class Topic(scriptRendering: ScriptRendering, language: Language = subhAShitaDb.Language("UNK"))
...
object quoteTextHelper {
  ...
  def getSanskritDevangariiQuote(text: String): QuoteText =
    new QuoteText(text = text, script=transliterator.scriptDevanAgarI, language = subhAShitaDb.Language("sa"))
}
...
object sourceHelper {
  ...
  def getSanskritDevanaagariiSource(title: String, authors: List[String]): Source = {
    return subhAShitaDb.Source(name=quoteTextHelper.getSanskritDevangariiQuote(title),
...
object quoteTest {
  ...
  def quoteTest: Unit = {
    ...
    val quoteText = subhAShitaDb.QuoteText(
      scriptRenderings = List(
        subhAShitaDb.ScriptRendering(text = "दण्डः शास्ति प्रजाः सर्वाः दण्ड एवाभिरक्षति। दण्डः सुप्तेषु जागर्ति दण्डं धर्मं विदुर्बुधाः।। \tदण्डः\t",
          scheme = transliterator.scriptDevanAgarI)),
      language = subhAShitaDb.Language("sa"))
    // implicit val formats = Serialization.formats(NoTypeHints)
    log info Serialization.writePretty(quoteText)
    val source = subhAShitaDb.Source(name=quoteTextHelper.emptyText)
    log info Serialization.writePretty(source)

    val origin = subhAShitaDb.OriginAnnotation(textKey = quoteText.key,
